...
function deterministicFeedback(
  ...
  return `Thanks for working through these marketing analytics cases.${overall?.value ? ` You scored ${overall.value}.` : ""} Strong answers here come from reasoning about the situation — looking at what customers actually do rather than the convenient story, weighing how reliable the evidence is, and resisting single-cause or 'obvious' explanations.`;
}
...
async function generateSubjectVariant(
  ...
  const system =
    "You are an assessment author writing ORIGINAL marketing analytics questions for an intro course. " +
  ...
  const user =
    `Write ${count} new, distinct marketing analytics case questions within the scope above.\n` +
    `For style only (do NOT copy these): ${JSON.stringify(examplePrompts)}.`;
  const out = await chatJson<{
