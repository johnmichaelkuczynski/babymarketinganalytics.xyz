...
export async function setupAuth(app: Express) {
  ...
  app.use(
    session({
      store: pgStore,
      secret:
        process.env.SESSION_SECRET || "basic-predictive-analytics-secret-key",
      resave: false,
  ...
  if (googleEnabled) {
    ...
    const getCallbackURL = () => {
      if (process.env.NODE_ENV === "production") {
        const prodDomain = (process.env.REPLIT_DOMAINS || "")
          .split(",")[0]
          ?.trim();
        // Modified: fallback is this app's deployed domain
        return `https://${prodDomain || "ed01b486-bd27-479f-a414-3c0170fc95d3-00-2utkqjompje1x.spock.replit.dev"}${CALLBACK_PATH}`;
      }
    ...
    // Modified: trustedHosts contains only this app's domains (no textsurgeonplus.xyz)
        ...
        async (accessToken, refreshToken, profile, done) => {
          try {
            ...
            if (!user) {
              ...
              user = await storage.updateUserGoogle(user.id, { displayName });
            }
