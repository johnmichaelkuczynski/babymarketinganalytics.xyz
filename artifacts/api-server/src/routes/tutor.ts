...
router.get("/tutor/suggestions/:lectureId", async (req, res): Promise<void> => {
  ...
  const SYSTEM_PROMPT =
    'You are a rigorous introductory marketing analytics tutor writing study questions. Reply as strict JSON of the form {"questions": string[]} with NO other keys.';
...
router.post("/tutor/ask", async (req, res): Promise<void> => {
  ...
  const sys =
    "You are an encouraging introductory marketing analytics tutor. Explain step by step, use clear examples and relatable cases, and define key terms (e.g. marketing analytics, customer data, segmentation, the average-customer myth, the funnel, conversion rate, the leak, customer lifetime value, acquisition cost, retention, churn, A/B testing, random split, attribution, last-click, personalization, the creepiness line, insight to campaign) when they come up. Keep replies short (3-6 sentences) unless the student asks for more detail. Never just give the answer — guide them.";
