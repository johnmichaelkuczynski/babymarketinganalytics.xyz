    ...
    steps.push(
      ...
      await run(
        ...
        async () => {
          ...
          // predictive analytics. Homework is scoped to its own topic's lecture, but
          ...
          const derived = await chatText(
            "You are a strong introductory predictive analytics student answering a short-answer problem. " +
          ...
          const verdict = await chatJson<{
            ...
            "You are an academic quality-control reviewer for a introductory predictive analytics course. " +
...
router.post("/diagnostics/expand-lectures", async (req, res) => {
  ...
  const sys =
    `You are a introductory predictive analytics lecturer producing the ${level.toUpperCase()} version of a lecture. ` +
